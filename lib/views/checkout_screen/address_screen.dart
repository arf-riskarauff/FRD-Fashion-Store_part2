...
class _AddressScreenState extends State<AddressScreen> {
    ...
    showModalBottomSheet(
      ...
      builder: (_) {
        return _AddressFormSheet(
          initial:
              initialAddress ?? (editIndex != null ? _addresses[editIndex] : null),
          onSave: (data) async {
            await FirestoreService.saveAddress(data, id: docId);
            if (!mounted) return;
            setState(() {
              if (editIndex != null) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        stream: FirestoreService.addressesStream(),
        builder: (context, snapshot) {
          final docs = snapshot.data?.docs ?? [];
          final remoteAddresses =
              docs.map((doc) => _addressFromData(doc.data())).toList();
          final addresses = remoteAddresses.isEmpty ? _addresses : remoteAddresses;

          return ListView(
            padding: const EdgeInsets.all(16),
            children: [
              ...addresses.asMap().entries.map((e) {
                final docId = e.key < docs.length ? docs[e.key].id : null;
                return _AddressTile(
                  address: e.value,
                  ...
                  onDelete: () async {
                    ...
                    if (!mounted) return;
                    setState(() {
                      if (e.key < _addresses.length) {
                        _addresses.removeAt(e.key);
                      }
                      if (_defaultIndex >= addresses.length - 1) {
                        _defaultIndex = 0;
                      }
                    });
                  },
  ...
  Map<String, dynamic> _addressFromData(Map<String, dynamic> data) {
    return {
      'tag': (data['tag'] ?? 'Home').toString(),
      'icon': Icons.home_rounded,
